...
function aggregateEvents(events) {
  ...
  for (const item of events) {
    ...
    if (isToday) summary.today.totalEvents += 1;
...
export async function recordEvent(event, params = {}, visitorId = null) {
  ...
  const payload = { at, event, params, visitorId };
  const filename = `${Date.now()}-${Math.random().toString(36).slice(2, 10)}.json`;
  const pathname = `${EVENT_PREFIX}${filename}`;

  await putWithRetry(pathname, JSON.stringify(payload), blobPutOptions(false));
  await appendEventToIndex({ ...payload, pathname });
