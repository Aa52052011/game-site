...
async function loadAllEvents() {
  ...
  try {
    ...
    // Index file exists (even when empty after reset) — do not fall back to legacy blobs.
...
function aggregateEvents(events) {
  ...
  const recordVisit = (path, visitorId, isToday) => {
    const key = `${visitorId || "anon"}:${path}`;
  ...
  for (const item of events) {
    ...
    switch (event) {
      case "page_view":
        recordVisit(path, visitorId, isToday);
        break;
      case "register_click": {
      ...
      case "scroll_depth": {
        ...
        if ([25, 50, 75, 100].includes(depth)) {
          ...
          if (depth === 25) recordVisit(path, visitorId, isToday);
        }
        break;
      }
      default:
