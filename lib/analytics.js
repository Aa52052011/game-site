...
async function postEvent(name, params, { mode = "fetch-then-beacon" } = {}) {
  ...
  // fetch-then-beacon: only one delivery path succeeds
...
export function trackPromoCopy(location = "promo_banner") {
  const params = { location, code: "1x_5224822", page_path: "/" };
  vercelTrack("promo_copy", params);
  sendImmediate("promo_copy", params);
}
